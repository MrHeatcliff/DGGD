...
sys.path.insert(0, str(ROOT / 'scripts/pacs_tuned'))
...
def worker(run, gpu, slot):
    ...
    check_dir=run/'preflight'/owner
    check_dir.mkdir(parents=True,exist_ok=True)
    environment=dict(os.environ,CUDA_VISIBLE_DEVICES=gpu,HF_HUB_OFFLINE='1')
    with (check_dir/'preflight.log').open('w') as log:
        check=subprocess.run([sys.executable,str(ROOT/'scripts/pacs_tuned/smoke.py'),
            '--output',str(check_dir),'--env',str(slot%4)],cwd=ROOT,env=environment,
            stdout=log,stderr=subprocess.STDOUT)
    if check.returncode:
        atomic_json(run/'PREFLIGHT_FAILED.json',{'owner':owner,'log':str(check_dir/'preflight.log')})
        (run/'STOP').touch()
    ...
    while not (run / 'STOP').exists() and not (run / 'COMPLETE').exists():
        ...
        with (out / f"train_attempt{task.get('attempt',1)}.log").open('w') as log:
            process = subprocess.Popen([sys.executable, str(ROOT/'scripts/pacs_tuned/train_trial.py'), str(claimed)],
