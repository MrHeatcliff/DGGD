...
def sample(trial, pair):
    algorithm, backbone = pair.split('--')
    ...
    if algorithm in ['CORAL','MMD']:
        h['mmd_gamma'] = trial.suggest_float('mmd_gamma',1e-3,1e2,log=True)
    elif algorithm=='IRM':
        h['irm_lambda'] = trial.suggest_float('irm_lambda',1e-1,1e5,log=True)
        h['irm_penalty_anneal_iters'] = trial.suggest_categorical('irm_penalty_anneal_iters',[0,100,500,1000,1500,2500])
    elif algorithm=='DANN':
        h['lr_g']=h['lr'];h['weight_decay_g']=h['weight_decay']
        h['lr_d']=trial.suggest_float('lr_d',1e-6,1e-3,log=True)
        h['lambda']=trial.suggest_float('lambda',1e-2,1e2,log=True)
        h['weight_decay_d']=trial.suggest_float('weight_decay_d',1e-7,1e-2,log=True)
        h['grad_penalty']=trial.suggest_categorical('grad_penalty',[0.,.01,.1,1.,10.])
        h['d_steps_per_g_step']=trial.suggest_categorical('d_steps_per_g_step',[1,2,4,8])
        h['beta1']=trial.suggest_categorical('beta1',[0.,.5])
        h['mlp_width']=trial.suggest_categorical('mlp_width',[128,256,512,1024])
        h['mlp_depth']=trial.suggest_categorical('mlp_depth',[3,4,5])
        h['mlp_dropout']=trial.suggest_categorical('mlp_dropout',[0.,.1,.5])
    return h
...
class Controller:
    def __init__(self, run, config):
        ...
        assert sum(p['gpus'] for p in config['pools']) == config['max_gpus'] and config['max_gpus'] in (4,8)
        ...
        if key not in self.state['studies']:
            study = optuna.create_study(study_name=key,direction='maximize',
                sampler=optuna.samplers.TPESampler(seed=self.config['sampler_seed']+env,
                    n_startup_trials=15,multivariate=True,constant_liar=True),
            ...
            baseline = {'lr':1e-5 if backbone.endswith('--dinov2') else 5e-5,
                'weight_decay':1e-4,'batch_size':32,'dropout':0.0}
            if not backbone.endswith('--dinov2'): baseline['freeze_bn']=True
            algorithm=backbone.split('--')[0]
            if algorithm in ['CORAL','MMD']:baseline['mmd_gamma']=1.
            if algorithm=='IRM':baseline.update(irm_lambda=100.,irm_penalty_anneal_iters=500)
            if algorithm=='DANN':baseline.update(lr_d=5e-5,weight_decay_d=1e-7,grad_penalty=0.,d_steps_per_g_step=1,beta1=.5,mlp_width=256,mlp_depth=3,mlp_dropout=0.,**{'lambda':1.})
    ...
    def ensure_pools(self, jobs):
        for index, pool in enumerate(self.config['pools']):
            ...
            command = ['sbatch','--parsable',f"--partition={pool['partition']}",
                '--job-name=pacs-methods-optuna',f"--gres=gpu:{pool['gpus']}",
                f"--cpus-per-task={8*pool['gpus']}",f"--mem={48*pool['gpus']}G",
                f"--time={pool['time']}",f'--chdir={ROOT}',
                f'--output={self.run}/slurm/pool-%j.log',str(ROOT/'scripts/pacs_tuned/pool.sbatch'),
                str(self.run),str(pool['gpus'])]
            if pool.get('nodelist'):command.insert(2,f"--nodelist={pool['nodelist']}")
    ...
    def process_results(self, jobs):
        for item in list(self.state['tasks'].values()):
            ...
            if (out/'failure.json').exists():
                if spec['phase']=='search' and (out/'NUMERICAL_FAILURE').exists():
                    self.get_study(spec['backbone'],spec['env']).tell(spec['trial_number'],state=TrialState.FAIL)
                    item['status']='FAILED_TRIAL';self.save();continue
                self.retry(item,'worker reported a nonzero exit');continue
            claims=list((self.run/'running').glob(spec['id']+'__*.json'))
            if claims:
                item['status']='RUNNING'
                job_id=claims[0].stem.rsplit('__',1)[1].split('_')[0]
            ...
            if spec['phase']=='search' and (out/'progress.json').exists():
                ...
                if p['step'] > item['last_report_step']:
                    ...
                    warmup=max(1500,spec['hparams'].get('irm_penalty_anneal_iters',0)+500)
                    if p['step']>=warmup and trial.should_prune() and p['step'] < self.config['steps']:
                        (out/'PRUNE').touch()
                    self.save()
    ...
    def fill(self, backbone):
        ...
        while active < self.config['max_gpus']:
            ...
            if waiting is None and self.state['stage']=='search':
                ...
                if counts[env]>=self.config['search_trials_by_pair'][backbone]: break
    ...
    def transition(self, backbone):
        ...
        if stage=='search':
            if len(tasks) < 4*self.config['search_trials_by_pair'][backbone]:return
    ...
    def collect(self, backbone):
        ...
        subprocess.run([sys.executable,str(ROOT/'scripts/pacs_tuned/collect.py'),str(self.run),
    ...
    def loop(self):
        while not (self.run/'STOP').exists():
            ...
            if self.state['backbone_index']>=len(self.config['backbones']):
                subprocess.run([sys.executable,str(ROOT/'scripts/pacs_tuned/collect.py'),str(self.run)],check=True)
...
def main():
    ...
    parser.add_argument('--run-dir',default='outputs/pacs_methods_optuna')
    parser.add_argument('--config',default='experiments/pacs_tuned/config.json')
